...
class DatabaseTests(unittest.TestCase):
    ...
    def test_init_db_creates_file(self):
        db_path = self.config_dir / "panel.db"
        self.assertTrue(db_path.exists())

    def test_connect_executes_query(self):
        with connect() as cur:
            result = cur.execute("SELECT 1 as x").fetchone()
    ...
    def test_audit_log_inserts(self):
        audit_log("testuser", "test.action", "ok", {"detail": "value"})
        with connect() as cur:
            row = cur.execute("SELECT * FROM audit_log WHERE action = ?", ("test.action",)).fetchone()
            self.assertIsNotNone(row)
            self.assertEqual(row["user"], "testuser")
            self.assertEqual(row["status"], "ok")

    def test_dns_zone_crud(self):
        with connect() as cur:
            ...
            zone = cur.execute("SELECT * FROM dns_zones WHERE domain = ?", ("example.com",)).fetchone()
            ...
            recs = cur.execute("SELECT * FROM dns_records WHERE zone_id = ?", (zone["id"],)).fetchall()
            self.assertEqual(len(recs), 1)
            self.assertEqual(recs[0]["type"], "A")

    def test_dns_zone_cascade_delete(self):
        with connect() as cur:
            cur.execute("INSERT INTO dns_zones (domain, soa_primary, soa_email, created_at) VALUES (?, ?, ?, ?)",
                        ("del.example.com", "ns1", "admin", "2026-01-01T00:00:00Z"))
            zone = cur.execute("SELECT * FROM dns_zones WHERE domain = ?", ("del.example.com",)).fetchone()
            cur.execute("INSERT INTO dns_records (zone_id, name, type, value, ttl, created_at) VALUES (?, ?, ?, ?, ?, ?)",
                        (zone["id"], "www", "A", "1.2.3.4", 3600, "2026-01-01T00:00:00Z"))
    ...
    def test_email_account_crud(self):
        with connect() as cur:
            ...
            acc = cur.execute("SELECT * FROM email_accounts WHERE domain = ?", ("example.com",)).fetchone()
            self.assertIsNotNone(acc)
            self.assertEqual(acc["mailbox"], "user")

    def test_database_record_crud(self):
        with connect() as cur:
            ...
            db = cur.execute("SELECT * FROM databases WHERE name = ?", ("mydb",)).fetchone()
            self.assertIsNotNone(db)
            self.assertEqual(db["db_type"], "mysql")

    def test_ssl_cert_crud(self):
        with connect() as cur:
            ...
            cert = cur.execute("SELECT * FROM ssl_certs WHERE domain = ?", ("ssl.example.com",)).fetchone()
