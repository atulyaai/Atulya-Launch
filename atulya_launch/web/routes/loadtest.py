...
@router.post("/run")
@require_admin
async def loadtest_run(request: Request, target: str = Form(...), requests: int = Form(10), concurrency: int = Form(2)):
    result = core.load_test(target, requests=requests, concurrency=concurrency)
...
@router.get("/api/run")
@require_admin
async def api_run(request: Request, target: str = "http://127.0.0.1:8080", requests: int = 5, concurrency: int = 2):
