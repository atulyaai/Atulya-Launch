...
async def email_list(request: Request):
    with connect() as cur:
        accounts = [dict(r) for r in cur.execute("SELECT id, domain, mailbox, quota_mb, created_at FROM email_accounts ORDER BY domain, mailbox").fetchall()]
...
@require_auth
async def account_create(request: Request, domain: str = Form(...), mailbox: str = Form(...), password: str = Form(...), quota_mb: int = Form(1024)):
...
@require_auth
async def account_delete(request: Request, account_id: int = Form(...)):
    with connect() as cur:
...
async def api_accounts(request: Request):
    with connect() as cur:
        accounts = [dict(r) for r in cur.execute("SELECT id, domain, mailbox, quota_mb, created_at FROM email_accounts").fetchall()]
