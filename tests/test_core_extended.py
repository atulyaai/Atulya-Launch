...
class CoreExtendedTests(unittest.TestCase):
    ...
    def test_nginx_apply_plan_returns_list(self):
        core.site_create("test.example.com")
        plan = core.nginx_apply_plan()
        self.assertEqual(len(plan), 1)
        self.assertEqual(plan[0]["domain"], "test.example.com")
        self.assertIn("source", plan[0])
        self.assertIn("target", plan[0])

    def test_nginx_apply_plan_single_domain(self):
        core.site_create("a.example.com")
        core.site_create("b.example.com")
        plan = core.nginx_apply_plan("a.example.com")
        self.assertEqual(len(plan), 1)
        self.assertEqual(plan[0]["domain"], "a.example.com")

    def test_nginx_apply_plan_missing_domain_raises(self):
        with self.assertRaises(ValueError):
            core.nginx_apply_plan("nonexistent.example.com")

    def test_security_scan_checks_bind_host(self):
        cfg = core.load_config()
        cfg["settings"]["bind_host"] = "0.0.0.0"
        core.save_config(cfg)
        result = core.security_scan()
        issues = [i for i in result["issues"] if i["check"] == "bind_host"]
        self.assertTrue(len(issues) > 0)

    def test_security_scan_checks_api_token(self):
        cfg = core.load_config()
        cfg["panel"]["api_token"] = None
        core.save_config(cfg)
        result = core.security_scan()
        issues = [i for i in result["issues"] if i["check"] == "api_token"]
        self.assertTrue(len(issues) > 0)

    def test_site_delete_removes_nginx_config(self):
        site = core.site_create("del.example.com")
    ...
    def test_file_write_and_read(self):
        ...
        entries = core.file_list("files.example.com", ".")
    ...
    def test_file_mkdir(self):
        ...
        entries = core.file_list("mkdir.example.com", ".")
    ...
    def test_file_delete(self):
        ...
        entries = core.file_list("delfile.example.com", ".")
        names = [e["name"] for e in entries]
        self.assertNotIn("todelete.txt", names)

    def test_file_delete_rejects_site_root(self):
    ...
    def test_backup_restore_roundtrip(self):
        ...
        backup = core.backup_create("roundtrip")
    ...
    def test_memory_status_returns_dict(self):
        result = core.memory_status()
        self.assertIn("total", result)
        self.assertIn("percent", result)

    def test_service_summary_returns_dict(self):
        result = core.service_summary()
        self.assertIsInstance(result, dict)

    def test_validate_domain_rejects_bad(self):
        with self.assertRaises(ValueError):
    ...
    def test_validate_domain_accepts_good(self):
    ...
    def test_site_create_duplicate_raises(self):
